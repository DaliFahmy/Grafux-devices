...
# from memory parameters), opengcram (the same for a gain-cell memory) and
# analogue_simulator (ngspice, transistor-level).  Six block types sharing one package, so they mount
...
try:
    from EDA.analogue_simulator_router import router as analogue_simulator_router
    ...
    from EDA.yosys_router import router as yosys_router

    app.include_router(verilator_router)
    ...
    app.include_router(analogue_simulator_router)
    logger.info("EDA routers mounted at /verilator, /yosys, /openroad, /openram, /opengcram and /analogue_simulator")
