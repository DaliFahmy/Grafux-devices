...
from . import composio_tools, connections
...
def _build_system_prompt(spec: ClawSpec, from_channel: bool = False) -> str:
    ...
    composio_desc = composio_tools.describe(spec)
    if composio_desc:
        # Real tools for the claw's connected Composio apps (executed via REST).
...
async def run_claw(
    ...
    composio_defs, composio_dispatch = await composio_tools.build(spec)
    ...
    try:
        if local_conns:
            ...
            text = await connections.run_local_agent_loop(
                ...
                extra_tool_defs=composio_defs, extra_dispatch=composio_dispatch,
            )
...
async def stream_claw(
    ...
    # Connection / tool paths aren't token-streamed — run them normally and emit once.
    if local_conns or mcp_servers or composio_tools.has_rest_tools(spec):
